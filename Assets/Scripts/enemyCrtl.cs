...
public class enemyCrtl : MonoBehaviour
{
    ...
    {
        //Take damage if hit, proportional to 1/30 of kinetic energy and the damage resistance, also take damage if hit wall too fast
        if (collision.GetComponent<Rigidbody2D>() != null)
        {
            ...
            {
                Health = Health - (Mathf.Pow(gameObject.GetComponent<Rigidbody2D>().velocity.x, 2) + Mathf.Pow(gameObject.GetComponent<Rigidbody2D>().velocity.y, 2)) * gameObject.GetComponent<Rigidbody2D>().mass / 60 * (1 - damageRes);
            }
            Health = Health - (Mathf.Pow(collision.GetComponent<Rigidbody2D>().velocity.x, 2) + Mathf.Pow(collision.GetComponent<Rigidbody2D>().velocity.y, 2)) * collision.GetComponent<Rigidbody2D>().mass / 60 * (1 - damageRes);
